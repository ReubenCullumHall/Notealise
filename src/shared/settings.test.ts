...
import { DEFAULT_UPDATE_PREFS, normalizeUpdatePrefs } from './update'
...
describe('normalizeUpdatePrefs', () => {
  ...
  it('ignores a stale betaChannel key from a config an older build wrote', () => {
    // the beta channel is gone; an old config that still carries it must import
    // cleanly, not be rejected
